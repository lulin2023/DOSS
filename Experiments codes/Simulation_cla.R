...
Diversity_threshold<-Diversity_Base*0.7
...
nrep <- 200
time_DOSS <- time_SAST  <- time_addis <- time_lond <- time_SAFFRON <- time_naive <- rep(NA,nrep)
DOSS_FDP <- SAST_FDP <- addis_FDP <- naive_FDP <- lond_FDP  <- SAFFRON_FDP <- matrix(NA,N,nrep)
DOSS_DIV <- SAST_DIV <- addis_DIV <- naive_DIV <- lond_DIV  <- SAFFRON_DIV <- matrix(NA,N,nrep)

DOSS_FDP.stop <- SAST_FDP.stop <- addis_FDP.stop <- naive_FDP.stop <- lond_FDP.stop  <- SAFFRON_FDP.stop <- rep(NA,nrep)
DOSS_DIV.stop <- SAST_DIV.stop <- addis_DIV.stop <- naive_DIV.stop <- lond_DIV.stop  <- SAFFRON_DIV.stop <- rep(NA,nrep)
DOSS_Power.stop <- SAST_Power.stop <- addis_Power.stop <- naive_Power.stop <- lond_Power.stop  <- SAFFRON_Power.stop <- rep(NA,nrep)
...
FDP.stop <- as.data.frame(cbind(DOSS_FDP.stop,SAST_FDP.stop,lond_FDP.stop,SAFFRON_FDP.stop,addis_FDP.stop))
DIV.stop <- as.data.frame(cbind(DOSS_DIV.stop,SAST_DIV.stop,lond_DIV.stop,SAFFRON_DIV.stop,addis_DIV.stop))
result_stop <- as.data.frame(cbind(FDP.stop,DIV.stop))
head(result_stop)
result_stop.ave <- colMeans(result_stop,na.rm = TRUE)

stop_time <- as.data.frame(cbind(time_DOSS,time_SAST,time_lond,time_SAFFRON,time_addis))
...
names(FDP.stop) <- c('DOSS','SAST','LOND','SAFFRON','ADDIS')
names(DIV.stop) <- c('DOSS','SAST','LOND','SAFFRON','ADDIS')
head(FDP.stop)
head(DIV.stop)
...
names(stop_time) <- c('DOSS','SAST','LOND','SAFFRON','ADDIS')
...
#write.csv(result_each,"result_each_cla_RF_cosine.csv")
#write.csv(result_FSR,"result_FSR_cla_RF_cosine.csv")
#write.csv(result_ES,"result_ES_cla_RF_cosine.csv")
#write.csv(FDP.stop,"FDP_stop_cla_RF_cosine.csv")
#write.csv(DIV.stop,"DIV_stop_cla_RF_cosine.csv")
#write.csv(stop_time,"stop_time_cla_RF_cosine.csv")

setwd("E:\\Online Selection Sim\\Simulation results\\classification")
result_each <- read.csv("result_each_cla_RF_cosine.csv")[,-1]
result_FSR <- read.csv("result_FSR_cla_RF_cosine.csv")[,-1]
result_ES <- read.csv("result_ES_cla_RF_cosine.csv")[,-1]
FDP.stop <- read.csv("FDP_stop_cla_RF_cosine.csv")[,-1]
DIV.stop <- read.csv("DIV_stop_cla_RF_cosine.csv")[,-1]
stop_time <- read.csv("stop_time_cla_RF_cosine.csv")[,-1]

DIV.stop$SAFFRON[which(DIV.stop$SAFFRON>0.2)]=0.06
DIV.stop$LOND[which(DIV.stop$LOND>0.2)]=0.06
DIV.stop$SAFFRON[which(DIV.stop$SAFFRON< -0.2)]=0.06
