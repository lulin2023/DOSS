...
#' @param X_test_scale: The center scaled covariates X
#' @param Alter_test: the index of true signal  (theta=1)
...
#' @returns the FSP (FDP) and ES (DIV) at every time t
#'
...
#' function to carry out the DOSS online sample selection procedure
#' @param X_test_scale The center scaled covariates X
#' @param Alter_test the index of true signal (theta=1)
#' @param TN The local FDR
#' @param alpha The FSR level alpha
#' @param Diversity_threshold The ES level K
#' @param Diversity_initial_num When the selection number exceeds the Diversity_initial_num, we consider computing diversity.
#' @param m the number of selection when stop
#' @param N the total number of observations
#' @param IsDiversity A binary variable. If IsDiversity==TRUE, the DOSS procedure is carried out, else the SAST procedure is carried out.
#' @returns A list contains FSP (FDP), ES (DIV), Power, decisions, select.num, and stoptime.
#'
...
# function to compute the selection result for some onlineFDR procedures, like LOND/LORD++/ADDIS/SAFFRON
